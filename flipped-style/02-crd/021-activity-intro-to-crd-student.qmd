...
```         
data <- read_csv("02_traffic_data.csv") |>
  mutate(TrafficSignal = factor(TrafficSignal, levels = c("Pretimed", "Semi-actuated", "Fully actuated")))
```

Then check your factor order associated with `i = 1, 2, 3`:
...
In context of the data, write out your conclusion from the overall ANOVA F-test from (c). Make sure to provide all supporting evidence -- (F = , df = , p = ).
